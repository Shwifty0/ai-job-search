...
5. **Page limit** - hard page count for the compiled PDF. Default: **2 pages** for a CV, **1 page** for a cover letter. `/apply`'s compile-and-inspect loop enforces this. For CVs, also record the **fill requirement**: the last page must be at least ~95% full (hard requirement, every time) - a page ending with a mostly-empty bottom third or more fails inspection exactly like an overflow does. Note this in the manifest's page-limit line so `/apply` restores content rather than leaving a thin-looking page.
